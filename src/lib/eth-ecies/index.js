 ...
 * @returns {Buffer} plaintext
 */
const DecryptV1 = (privKey, encrypted, opts) => {
  opts = opts || {};
  let ephemPubKeyEncoded; let
    ephemPubKey;
  let offset = 0;
  if (opts.ephemPrivKey) {
    offset = 65;
    const ephemPrivKey = ec.keyFromPrivate(Buffer.from(opts.ephemPrivKey, 'hex'));
    ephemPubKey = ephemPrivKey.getPublic();
    ephemPubKeyEncoded = Buffer.from(ephemPubKey.encode());
  } else {
