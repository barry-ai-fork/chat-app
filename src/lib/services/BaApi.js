...
import { CoinApi } from '../bootstrap';

export class MiniTicker {
  constructor({
    e, E, s, c, o, h, l, v, q,
  }) {
    this.event = e;
    this.time = Number(E); // unixtimestamp
    this.symbol = s;
    this.close = Number(c);
    this.open = Number(o);
    this.high = Number(h);
    this.low = Number(l);
    this.volumn = Number(v);
...
export class BaApi {
  static async fetchKLineData({
    symbol, startTimeSec, endTimeSec, limit, interval,
  }) {
    const { data } = await axios.get(CoinApi('/ba-api'), {
      params: {
        path: 'v3/klines',
        symbol,
        interval,
  ...
  static parseIntervalSec(interval) {
    ...
    const n = parseInt(interval.substring(0, interval.length - 1), 10);
    switch (t) {
      case 'm':
        return 60 * n;
      case 'h':
